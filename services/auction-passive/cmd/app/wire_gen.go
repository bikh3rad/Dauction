...
func wireApp(ctx context.Context) (app.Application, error) {
	...
	auction := repo.NewAuction(logger, postgresDB)
	creditDebitor := repo.NewCreditDebitor(logger, kConfig)
	bizAuction := biz.NewAuction(logger, auction, creditDebitor)
	auctionHandler := handler.NewAuctionHandler(logger, serveMux, bizAuction)
	...
	eventConsumer := biz.NewEventConsumer(logger, bizAuction)
	runner := eventbus.NewRunner(logger, nats, outboxPublisher, eventConsumer, controller)
	v := handler.NewServiceList(healthzHandler, auctionHandler, runner)
