...
func wireApp(ctx context.Context) (app.Application, error) {
	...
	escrowRepo := repo.NewEscrow(logger, postgresDB)
	escrow := biz.ProvideEscrow(logger, escrowRepo)
	escrowHandler := handler.NewEscrowHandler(logger, serveMux, escrow)
	...
	eventConsumer := biz.NewEventConsumer(logger, escrow)
	runner := eventbus.NewRunner(logger, nats, outboxPublisher, eventConsumer, controller)
	v := handler.NewServiceList(healthzHandler, escrowHandler, runner)
