...
> **Service: `gateway`** — the Dauction edge. Single public entrypoint that
> reverse-proxies every `/apis/*` route to the owning backend service after authN,
> a tier/KYC guard and rate-limiting. The repo-root `CLAUDE.md` is binding
> (topology §2, routes §6, conventions §0). The template plumbing below (Wire,
> koanf, OTel, vertical slice) still applies.

## Gateway service map

**Owns no domain DB. Stateless.** No Postgres, no NATS, no Redis — the datasource
layer is emptied and `DataProviderSet` is an empty `wire.NewSet()`. The only
external dependency is identity's read model (over HTTP) for the guard.

**Vertical slice (the one resource is "access"):**
`entity.Access` → `biz.UsecaseAccess` (+ `RepositoryAccess` seam) → `biz.access`
impl (guard decision + brief cache) → `repo.identityAccess` (HTTP client to
identity) → `handler.ProxyHandler` (the edge). The proxy engine lives in
`internal/proxy` and the route table in `internal/router`.

**Route table (`internal/router/routes.go`).** Longest-prefix-wins with
suffix/exact specificity. Maps each `/apis/*` prefix to a logical upstream name
(resolved via `app.UpstreamsConfig`) and a `RouteRequirement` (Public / authed /
participation). Full table is in the README.

- **Dutch vs Passive disambiguation:** both engines share `/apis/auctions/{id}/`.
  They are split by **action suffix** — `reserve|lock|buy` → `auction-dutch`,
  `bid|standing` → `auction-passive`; admin `open|complete|abort` → dutch,
  `close` → passive. (A cleaner long-term design is `/apis/auctions/dutch/...` vs
  `/apis/auctions/passive/...`; this matches the frozen routes in root §6.)
- **Escrow vs Dispute:** `/apis/escrow/{id}/dispute/resolve` → `dispute`; all other
  `/apis/escrow/*` and `/apis/admin/escrow/*` → `escrow`.

**Middleware chain (`ProxyHandler.RegisterHandler`, outermost → innermost):**
request-id/logging → panic recovery → **auth** (strip spoofed `X-Account-*`
headers, parse bearer, inject trusted `X-Account-Id`) → **rate-limit**
(fixed-window, keyed by resolved account else client IP) → route match → **tier/KYC
guard** (`biz.UsecaseAccess.Authorize`) → trusted-header injection
(`X-Account-Tier`, `X-Kyc-Approved`) → reverse-proxy. Auth precedes the limiter so
it can key on the account; both run after recovery.

**Auth (dev scheme):** `Authorization: Bearer <accountId>` — the bearer token IS
the account UUID (real JWT validation is future work; the seam is
`middlewares.extractAccountID`). Inbound `X-Account-Id` / `X-Account-Tier` /
`X-Kyc-Approved` are always stripped before resolution — never trust client
identity headers.

**Guard read model:** the guard calls identity `GET /apis/internal/accounts/{id}/access`
via the `RepositoryAccess` seam, caches the result ~5s, and authorizes per route:
public bypasses; participation routes require `tier∈{MEMBER,VIP} ∧ kyc=APPROVED`
(root §1) → else `TIER_REQUIRED` / `KYC_REQUIRED` (403); unauthenticated callers on
non-public routes → `UNAUTHORIZED` (401).

**Errors:** language-neutral codes via `dto.HandleError` (sentinel → code+status)
and `dto.HandleErrorCode` (bare code, for middlewares). Codes: `UNAUTHORIZED`,
`TIER_REQUIRED`, `KYC_REQUIRED`, `RATE_LIMITED`, `UPSTREAM_UNAVAILABLE`,
`NOT_FOUND`, `RESOURCE_INVALID`, `INTERNAL`.

**Realtime:** `/apis/live/*` → `notifier`; `httputil.ReverseProxy` passes
`Connection: Upgrade` through unmodified, so WebSocket/SSE streams work.

**Config env (koanf `APP_`-prefixed):** `APP_SERVER_HTTP_ADDR` (`:8080`),
`APP_UPSTREAMS_*` (11 service URLs), `APP_ACCESS_BASEURL` / `APP_ACCESS_TIMEOUTSECONDS`,
`APP_RATELIMIT_ENABLED|LIMIT|WINDOWSECONDS` (default 100/10s),
`APP_COLLECTOR_EXPORTERS_OTLP_ENDPOINT`. Defaults target `deploy/docker-compose.yml`.
