...
func wireApp(ctx context.Context) (app.Application, error) {
	...
	vaultRepo := repo.NewVault(logger, postgresDB)
	vault := biz.NewVault(logger, vaultRepo)
	vaultHandler := handler.NewVaultHandler(logger, serveMux, vault)
	...
	eventConsumer := biz.NewEventConsumer(logger, vault)
	runner := eventbus.NewRunner(logger, nats, outboxPublisher, eventConsumer, controller)
	v := handler.NewServiceList(healthzHandler, vaultHandler, runner)
