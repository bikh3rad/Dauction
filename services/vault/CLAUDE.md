...
## Vault service (Dauction)

Bounded context: **members' private collections, instant buyback, the Vault-Credit ledger, and
list-to-auction**. Owns its `vault` Postgres DB and integrates only through events (NATS/JetStream)
and each owner's sync API — never DB→DB (root `CLAUDE.md` §0–§2). The template plumbing docs below
still apply (Wire, koanf, OTel).

**Tables (owned, `migrations/`):**
- `vault_object` — `id` (UUID PK), `owner_account_id` (UUID), `title`, `description`,
  `appraised_value_cents` (BIGINT, USDC cents, CHECK > 0), `state`
  (`IN_VAULT|APPRAISING|IN_AUCTION|SOLD|BOUGHT_BACK`, CHECK), timestamps. Index `(owner, created_at DESC)`.
- `vault_credit_ledger` — append-only; `id`, `account_id`, `delta_cents` (BIGINT signed, USDC cents
  of **Vault Credit**, NOT bid credits), `reason` (`BUYBACK|AUCTION_RELEASE|ADJUSTMENT`, CHECK),
  `ref_id`, `created_at`. Balance = `SUM(delta_cents)`. Index `(account_id)`.
...
- `GET  /apis/vault` — caller's objects + Vault-Credit balance (cents).
- `POST /apis/vault/objects` — add an object `{title, description, appraisedValueCents}` → `IN_VAULT`.
- `POST /apis/vault/objects/{id}/list` — `{atype: DUTCH|VICKREY|UNIQBID, durationDays?: 2|5|7}`.
  `durationDays` REQUIRED for VICKREY/UNIQBID, FORBIDDEN for DUTCH (violation → `ErrResourceInvalid`).
  `IN_VAULT → APPRAISING`; emits `object.listed`.
- `POST /apis/vault/objects/{id}/buyback` — `{mode: CASH|CREDIT}`. CASH = 50% appraised (USDC, settled
  off-ledger); CREDIT = 85% appraised credited to the ledger + emits `credit.changed`. `IN_VAULT → BOUGHT_BACK`.
- Owner-only: acting on another member's object → `ErrResourceAccessDenied`.

**State machine (enforced in `biz`; illegal → `ErrResourceInvalid`):**
`IN_VAULT → APPRAISING → IN_AUCTION → SOLD`, plus terminal `IN_VAULT → BOUGHT_BACK`. Transitions are
atomic conditional UPDATEs in the repo (`WHERE id = $ AND state = from`); a non-matching row → invalid.

**Money rule:** all amounts are `int64` USDC cents. Buyback percentages use integer math with
**truncation toward zero** (`value*50/100`, `value*85/100`). Vault Credit is USDC-denominated cents,
never bid credits (root §5).

**Events emitted (outbox → NATS):** `object.listed` `{object_id, owner_id, mode, duration, floor}`,
`credit.changed` `{account_id, delta, balance, reason}`. Both commit in the SAME tx as the state/ledger
write; a background `OutboxPublisher` relays unpublished rows and marks them published.

**Events consumed (inbox-idempotent):** `auction.completed` → owned `IN_AUCTION → SOLD`; when the
payload marks the seller release as Vault Credit, append an `AUCTION_RELEASE` ledger row + emit
`credit.changed`. Non-`COMPLETED` final states, unknown objects, and already-terminal objects are
no-op successes; duplicates dedup on the subject-scoped `idempotency_key`.

**Atomic credit + event:** `BuybackTx`/`SettleSoldTx` take an `OutboxBuilder func(balanceCents) (OutboxEvent, error)`
that the repo invokes INSIDE the transaction (after the post-write balance is known), so the
`credit.changed` event commits with its ledger row.

**Config env (koanf `APP_`-prefixed):** `APP_DATASOURCE_POSTGRES_DSN` (db `vault`, host `pg-vault:5432`),
`APP_DATASOURCE_NATS_DSN` (`nats:4222`, stream `DAUCTION`, durable `vault`,
subjects `object.listed,credit.changed,auction.completed`), `APP_SERVER_HTTP_ADDR` (`:8080`),
