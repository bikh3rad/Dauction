...
// PostgresConfig is the datasource.postgres config sub-tree.
type PostgresConfig struct {
	DSN string `koanf:"dsn"`
}

func NewPostgresDB(ctx context.Context, logger *slog.Logger, controller app.Controller, config *app.KConfig) (*PostgresDB, error) {
	cfg := new(PostgresConfig)
	if err := config.Unmarshal("datasource.postgres", cfg); err != nil {
		return nil, err
	}

	dsn := cfg.DSN
	if dsn == "" {
		dsn = "postgresql://dauction:dauction@localhost:5432/invite?sslmode=disable"
	}
