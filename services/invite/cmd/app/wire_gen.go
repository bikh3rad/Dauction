...
func wireApp(ctx context.Context) (app.Application, error) {
	...
	invite := repo.NewInvite(logger, postgresDB)
	inviteConfig, err := biz.NewInviteConfig(kConfig)
	if err != nil {
		return nil, err
	}
	bizInvite := biz.NewInvite(logger, invite, inviteConfig)
	handlerInvite := handler.NewInvite(logger, serveMux, bizInvite)
	nats, err := datasource.NewNats(ctx, logger, kConfig)
	if err != nil {
		return nil, err
	}
	outboxPublisher := repo.NewOutboxPublisher(logger, postgresDB, nats, controller)
	v := handler.NewServiceList(healthzHandler, handlerInvite, outboxPublisher)
