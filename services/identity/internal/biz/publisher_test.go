...
func TestOutboxPublisher_PublishesAndMarks(t *testing.T) {
	...
	ev := entity.OutboxEvent{
		ID:             uuid.New(),
		Subject:        biz.SubjectAccountTierChanged,
		IdempotencyKey: "key-1",
		Payload:        []byte(`{"type":"account.tier_changed"}`),
	}
...
func TestOutboxPublisher_StopsOnPublishError(t *testing.T) {
	...
	ev := entity.OutboxEvent{ID: uuid.New(), Subject: "account.tier_changed", Payload: []byte(`{}`)}
