...
## Identity service (Dauction)

Bounded context: **accounts, access tiers, and the participation read model** for the gateway guard.
This service owns its `identity` Postgres DB and reaches other services only via events/their APIs —
never DB→DB (root `CLAUDE.md` §0–§2). The template plumbing docs below still apply (Wire, koanf, OTel).

**Tables (owned, `migrations/`):**
- `account` — `id` (UUID PK), `tier` (`GUEST|MEMBER|VIP`, CHECK), `kyc_status`
  (`PENDING|APPROVED|REJECTED`, CHECK), `created_at`, `updated_at`. Index `(tier, kyc_status)`.
- `outbox` — transactional outbox; `id`, `subject`, `idempotency_key` (UNIQUE), `payload` (JSONB),
  `created_at`, `published_at`. Partial index on unpublished rows.
...
- `GET  /apis/me` — current account (tier, KYC, `eligible`). Subject from `X-Account-Id` (gateway-injected).
- `GET  /apis/internal/accounts/{id}/access` — minimal tier+KYC+eligible read for the gateway guard.
- `POST /apis/admin/accounts/{id}/vip` — house grants VIP.
...
no-op successes (never lower a tier). KYC mirror is monotone-on-approve. Enums are MONOSPACE_UPPERCASE
strings (the value string IS the wire code). No money in this service.

**Config env (koanf `APP_`-prefixed):** `APP_DATASOURCE_POSTGRES_DSN` (db `identity`),
...
(`pg-identity:5432`, `nats:4222`, `jaeger:4317`).
