...
// Nats is the JetStream client. The notifier owns no domain DB and is a pure
// consumer: it binds a durable consumer over the shared Dauction event stream and
// projects each event into broadcast view-state. It never publishes.
type Nats struct {
	logger   *slog.Logger
	conn     *nats.Conn
...
// subjects the notifier consumes; Durable is the consumer durable name.
...
// NewNats connects to NATS, ensures the shared event stream exists (idempotently),
// and registers healthz + shutdown on the controller.
...
// stop function. The notifier projects view-state on each delivery; an unknown
// subject is acked as a no-op by the consumer.
func (n *Nats) Consume(
	ctx context.Context,
