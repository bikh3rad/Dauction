...
func wireApp(ctx context.Context) (app.Application, error) {
	...
	dispute := repo.NewDispute(logger, postgresDB)
	bizDispute := biz.NewDispute(logger, dispute)
	disputeHandler := handler.NewDisputeHandler(logger, serveMux, bizDispute)
	...
	eventConsumer := biz.NewEventConsumer(logger)
	runner := eventbus.NewRunner(logger, nats, outboxPublisher, eventConsumer, controller)
	v := handler.NewServiceList(healthzHandler, disputeHandler, runner)
