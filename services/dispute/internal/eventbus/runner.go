...
func (r *Runner) start(ctx context.Context) error {
	...
	// dispute is publish-only (it emits dispute.opened / dispute.resolved). It has
	// no inbound subscription, so we bind one only when the consumer declares
	// subjects (binding an empty FilterSubjects set would error on JetStream).
	subjects := r.consumer.Subjects()
	if len(subjects) > 0 {
		stop, err := r.nats.Consume(runCtx, "dispute", subjects, r.consumer.Handle)
		if err != nil {
			cancel()
	...
	r.logger.InfoContext(ctx, "eventbus runner started", "subjects", subjects)
