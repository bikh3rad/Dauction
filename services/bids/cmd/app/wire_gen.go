...
func wireApp(ctx context.Context) (app.Application, error) {
	...
	wallet := repo.NewWallet(logger, postgresDB)
	bizWallet := biz.NewWallet(logger, wallet)
	walletHandler := handler.NewWalletHandler(logger, serveMux, bizWallet)
	...
	runner := eventbus.NewRunner(logger, outboxPublisher, controller)
	v := handler.NewServiceList(healthzHandler, walletHandler, runner)
