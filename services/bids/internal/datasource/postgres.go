...
// PostgresConfig is the datasource.postgres koanf sub-tree.
type PostgresConfig struct {
	DSN string `koanf:"dsn"`
}

func NewPostgresDB(ctx context.Context, logger *slog.Logger, config *app.KConfig, controller app.Controller) (*PostgresDB, error) {
	cfg := new(PostgresConfig)
	if err := config.Unmarshal("datasource.postgres", cfg); err != nil {
		logger.Error("failed to unmarshal postgres config", "error", err)
