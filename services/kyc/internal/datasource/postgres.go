...
// PostgresConfig is the koanf-bound config for the Postgres datasource.
type PostgresConfig struct {
	DSN string `koanf:"dsn"`
}

func NewPostgresDB(ctx context.Context, logger *slog.Logger, controller app.Controller, kcfg *app.KConfig) (*PostgresDB, error) {
	cfg := new(PostgresConfig)
	if err := kcfg.Unmarshal("datasource.postgres", cfg); err != nil {
		return nil, err
	}

	dsn := cfg.DSN
	if dsn == "" {
		dsn = "postgresql://dauction:dauction@localhost:5432/kyc?sslmode=disable"
	}
